...
class MASTER(nn.Module):
    def __init__(self, d_feat, d_model, t_nhead, s_nhead, T_dropout_rate, S_dropout_rate, gate_input_start_index, gate_input_end_index, beta):
        ...
        self.feature_gate = Gate(self.d_gate_input, self.gate_input_start_index, beta=beta)
        ...
        self.layers = nn.Sequential(
            # feature layer
            nn.Linear(self.gate_input_start_index, d_model),
...
#
#         self.model = MASTER(
#             d_feat=self.d_feat, d_model=self.d_model,
#             t_nhead=self.t_nhead, s_nhead=self.s_nhead,
...
class MASTERModel(SequenceModel):
    ...
    def init_model(self):
        self.model = MASTER(d_feat=self.d_feat, d_model=self.d_model, t_nhead=self.t_nhead, s_nhead=self.s_nhead,
                                   T_dropout_rate=self.T_dropout_rate, S_dropout_rate=self.S_dropout_rate,
                                   gate_input_start_index=self.gate_input_start_index,
                                   gate_input_end_index=self.gate_input_end_index, beta=self.beta)#实际的神经网络模型
        # self.model.load_state_dict(torch.load("100E.pt"))
        torch.serialization.add_safe_globals([MASTERModel])
